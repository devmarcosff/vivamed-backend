...
export class CidadaoService {
  ...
  async findAll() {
    const cidadoes = await this.cidadaoRepository.find({ order: { createAt: 'DESC' }, relations: { consultas: true, agendaConsultas: true } });
  ...
  async findByProntuario(prontuario: any) {
    const cidadoes = await this.cidadaoRepository.findOne({ order: { createAt: 'DESC' }, where: { prontuario: prontuario }, relations: { consultas: true, agendaConsultas: true } });
